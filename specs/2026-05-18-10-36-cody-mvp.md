...
- `kustomization.yaml`
  - Is included from `non-prod/kustomization.yaml` through the `kelos` resource entry.
...
- `../../bases/kelos/controller`
  - Creates namespace `kelos-system`.
  - Defines HelmRepository `kelos` from `oci://ghcr.io/kelos-dev/charts`.
  - Defines HelmRelease `kelos`, chart `kelos`, version `0.33.0`.
  - Cody currently depends on this base but patches it because `0.33.0` does not include the Slack reporter behavior and RBAC needed by the current Cody flow.
...
- `external-secret-slack.yaml`
  ...
  - Remote keys:
    - `cody-slack-bot-token`
    - `cody-slack-app-token`
...
- `external-secret-cody-github-app.yaml`
  ...
  - Remote keys:
    - `cursor-github-app-client-id`
    - `cursor-github-app-installation-id`
    - `cursor-github-app-private-key`
...
- `external-secret-github.yaml`
  ...
  - Inlines public GitHub App ID `3429269` into the rendered Kubernetes Secret.
  - Remote keys:
    - `cursor-github-app-installation-id`
    - `cursor-github-app-private-key`
    - `cody-github-webhook-secret`

- `httproute.yaml`
  - Exposes the Kelos GitHub webhook endpoint at `cody.qwlth.dev/webhook/github`.
  - Attaches to parent Gateway `north-south-gateway` in namespace `north-south-gw`.
  - Routes to backend Service `kelos-webhook-github` on port `8443`.
  - This is currently wired, but Slack is the primary Cody MVP path.

### Platform GitOps Traceability Matrix

| Platform GitOps object | Spec coverage |
| --- | --- |
| `non-prod/kustomization.yaml` | Includes `kelos`, which activates the Cody platform layer in non-prod. |
| `bases/kelos/controller/namespace.yaml` | Captured as the `kelos-system` namespace dependency. |
| `bases/kelos/controller/release.yaml` | Captured as the Kelos chart source/version dependency and reason for temporary patches. |
| `non-prod/kelos/kustomization.yaml` | Captured as the Cody resource aggregator. |
| `non-prod/kelos/helmrelease-patch.yaml` | Captured as Slack/GitHub webhook enablement plus image overrides. |
| `non-prod/kelos/taskspawner-cody-debug.yaml` | Captured as the Slack-to-Codex TaskSpawner. |
| `non-prod/kelos/agentconfig-cody-debugger.yaml` | Captured as Cody's behavioral runbook and Context7 MCP wiring. |
| `non-prod/kelos/rbac-cody-debugger.yaml` | Captured as read-only cluster diagnostic access. |
| `non-prod/kelos/rbac-cody-debugger-secrets.yaml` | Captured as tenant-only Secret read access, including the current namespace list. |
| `non-prod/kelos/rbac-slack-server-extra.yaml` | Captured as temporary Slack reporter RBAC while running main-built Kelos. |
| `non-prod/kelos/external-secret-slack.yaml` | Captured with rendered secret and Key Vault key names. |
| `non-prod/kelos/external-secret-codex.yaml` | Captured with rendered secret and Key Vault key name. |
| `non-prod/kelos/external-secret-cody-github-app.yaml` | Captured with rendered secret and reused cursor Key Vault key names. |
| `non-prod/kelos/external-secret-github.yaml` | Captured with rendered webhook secret, App ID, and Key Vault key names. |
| `non-prod/kelos/httproute.yaml` | Captured with hostname, path, parent Gateway, backend service, and port. |
