...
The currently deployed Cody MVP is an internal Slack-triggered autonomous background agent for non-prod platform debugging and small reviewable fixes.

Today, a user mentions `@cody` in Slack with a service issue or platform question. Kelos creates a one-off Codex task pod. That pod runs with Cody-specific instructions, read-only Kubernetes access, GitHub App credentials, cluster/debug tooling, and Alpheya engineering skills. Cody investigates, replies once in the Slack thread, and opens a pull request when a code or GitOps fix is appropriate.

The intended product shape is similar to Devin-style background engineering work, but narrower and infrastructure-owned:

- Slack is the only Cody trigger currently deployed.
...
## Trigger and Signal Surfaces

The current workspace scan shows three distinct layers: what Kelos supports, what the Alpheya Cody deployment actually enables, and which org observability systems could become inputs.

```mermaid
flowchart LR
  subgraph Live["Cody deployed today"]
    Slack["Slack @cody mention"] --> Cody["cody-debug-slack TaskSpawner"]
    GHRoute["cody.qwlth.dev/webhook/github"] -. "webhook server and route only" .-> NoGH["no Cody githubWebhook TaskSpawner found"]
  end

  subgraph KelosSupported["Kelos trigger types supported by CRD"]
    GHIssues["GitHub issues polling"]
    GHPRs["GitHub PR polling"]
    GHWebhook["GitHub webhooks"]
    Cron["Cron schedules"]
    Jira["Jira polling"]
    Linear["Linear webhooks"]
    Generic["Generic JSON webhooks"]
    SlackSource["Slack"]
  end

  subgraph OrgSignals["Org observability and workflow signals"]
    Datadog["Datadog monitors, SLOs, synthetics, deploy events"]
    Grafana["Grafana, Prometheus, Mimir, Alertmanager"]
    Otel["OTel, Tempo, Loki, Alloy"]
    Policy["Kyverno policy reporter"]
    Assay["Assay OTel and k8s tools"]
  end

  OrgSignals -. "future: generic webhook or alert receiver" .-> Generic
  Generic -. "future Cody TaskSpawner" .-> Cody
```

### Trigger Matrix

| Surface | Kelos support | Deployed for Cody today | Current read |
| --- | --- | --- | --- |
| Slack mentions | Yes, `when.slack` | Yes | `non-prod/kelos/taskspawner-cody-debug.yaml` fires on every `@cody` mention. |
| GitHub webhook | Yes, `when.githubWebhook` | Partially wired, not active for Cody | `kelos-webhook-github` and `cody.qwlth.dev/webhook/github` exist, but no Cody TaskSpawner uses `githubWebhook`. |
| GitHub issues polling | Yes, `when.githubIssues` | No | Present in Kelos CRD/docs/examples, not in Alpheya Cody GitOps. |
| GitHub PR polling | Yes, `when.githubPullRequests` | No | Present in Kelos CRD/docs/examples, not in Alpheya Cody GitOps. |
| Cron | Yes, `when.cron` | No | Kelos self-development manifests use cron, but Cody has no deployed cron TaskSpawner. |
| Jira | Yes, `when.jira` | No | Supported by Kelos polling, not configured for Cody. |
| Linear webhook | Yes, `when.linearWebhook` | No | Supported by Kelos webhook server, not configured for Cody. |
| Generic webhook | Yes, `when.webhook` | No | Suitable for Sentry, PagerDuty, Datadog, Alertmanager, or internal JSON events, but generic webhook source is not enabled in Cody Helm values. |
| Observability tools | Through generic webhook or Slack relay | No direct Cody trigger | Existing tools alert or expose data, but none currently create Cody Tasks automatically. |

Kelos self-development manifests in this fork do use cron and GitHub webhook TaskSpawners. Those are good reference implementations, but they are not part of the current Alpheya Cody deployment in `k8s-platform-gitops`.

### Observability Integration Options

```mermaid
flowchart TD
  Alert["Datadog, Alertmanager, Sentry, PagerDuty, internal service"] -->|"POST JSON event"| GenericEndpoint["kelos generic webhook<br/>/webhook/<source>"]
  GenericEndpoint --> Spawner["Cody TaskSpawner<br/>when.webhook"]
  Spawner --> Agent["Codex task pod"]
  Agent --> Evidence["Kubernetes, logs, OTel/Datadog links, GitOps"]
  Agent --> Reply["Slack reply or PR"]

  ExistingAlerts["Current alert channels"] -->|"Slack notification"| Human["Engineer"]
  Human -->|"@cody with context"| SlackCody["Slack TaskSpawner"]
  SlackCody --> Agent
```

| Org signal | Exists today | Cody relationship |
| --- | --- | --- |
| Datadog | Platform GitOps has Datadog operator/agent and Flux deploy events. Apps GitOps has Datadog instrumentation and tools for monitors, SLOs, synthetics, and performance reports. | Strong future input. A monitor webhook could open a Cody investigation with service/env/tags. No direct Cody binding exists today. |
| Grafana, Prometheus, Mimir, Alertmanager | Platform GitOps has monitoring stacks and Alertmanager configs that post to Slack alert channels. | Current path is human-in-the-loop: alert lands in Slack, human mentions Cody. A webhook receiver could make this automatic. |
| OTel, Tempo, Loki, Alloy | Platform and app GitOps include OTel collectors, instrumentation, tracing, logging, Tempo, Loki, and Alloy. | Best used as evidence during investigation. Not an event source by itself unless paired with alerts. |
| Kyverno policy reporter | Platform GitOps sends policy reporter violations to Slack. | Could become a Cody trigger for policy drift or admission-policy follow-up, but today it is Slack alerting only. |
| Assay | Separate agent system with Slack and cron triggers plus `query_otel`, `kubectl`, DB, Jira, and GitHub tools. | Useful reference for observability-aware agent design. It is not wired to Cody/Kelos in the scanned repos. |

Important caveat: the Kelos docs in this fork warn that the generic webhook endpoint is currently unauthenticated, even though Helm values reserve a secret name for future HMAC validation. Any Cody observability webhook MVP should either add signature validation first or keep the endpoint network-restricted.
...
12. Decide whether cron-triggered Cody maintenance checks belong in MVP or a later operational phase.
13. Decide whether observability-triggered Cody runs should start with Datadog, Alertmanager, or Sentry-style generic webhooks.
14. Add authentication or network restriction before enabling Kelos generic webhooks for Cody.
15. Add a kill switch or suspend procedure for runaway Slack or webhook triggering.
...
- Should Cody add a cron TaskSpawner for periodic non-prod health checks, or should scheduled runs live in Assay instead?
- Which observability system should be the first automatic trigger: Datadog monitor webhook, Alertmanager webhook, Kyverno policy reporter, Sentry-style error events, or none until Slack is stable?
...
## Appendix: PromptTemplate vs AgentConfig

Kelos has two separate instruction surfaces, and they should not be treated as duplicates.

| Surface | Mental model | What it should contain | Cody example |
| --- | --- | --- | --- |
| `TaskSpawner.spec.taskTemplate.promptTemplate` | The per-run request | Source-specific payload and task-specific instructions. It is rendered into `Task.spec.prompt` using variables like `{{.Body}}`, `{{.URL}}`, `{{.Title}}`, `{{.Time}}`, or `{{.Schedule}}`. | "Someone pinged you in Slack. Here is the Slack message and thread URL. Reply once." |
| `AgentConfig.spec.agentsMD` | The reusable operating manual | Stable role, workflows, policies, tool guidance, output expectations, MCP servers, skills, and shared behavior that many tasks can reuse. | "You are Cody, an Alpheya platform helper. Use this debug workflow. Follow these repo lookup rules. Use read-only cluster access. Open PRs only when evidence-backed." |

Short version:

```text
AgentConfig = who the agent is, what tools it has, and how it should generally work.
promptTemplate = what happened this time and what the agent should do for this one run.
```
...
```mermaid
flowchart LR
  Source["Slack/GitHub/Cron/Webhook event"] --> Spawner["TaskSpawner"]
  Spawner --> Prompt["Render promptTemplate with event fields"]
  Spawner --> Task["Create Task"]
  Prompt --> Task
  Task --> Controller["Kelos controller"]
  Controller --> Config["Resolve AgentConfig"]
  Config --> Env["Inject KELOS_AGENTS_MD / plugins / skills / MCP"]
  Env --> Pod["Agent pod"]
  Task --> Pod
  Pod --> Agent["Codex/Claude/Gemini run"]
```

### Why Both Exist

Use `promptTemplate` when the instruction depends on the trigger payload:

- Slack message body and thread URL.
- GitHub issue title, body, comments, labels, and URL.
- Cron schedule and tick time.
- Generic webhook fields mapped from JSON.

Use `AgentConfig` when the instruction should stay true across many tasks:

- Cody's identity and scope.
- Debugging workflow.
- Repo mapping rules.
- Safety limits.
...
This lets multiple task spawners share one Cody "brain" while still passing different event-specific requests. For example, Slack, GitHub, and cron could all point at a shared `cody-debugger` AgentConfig, but each would have a different `promptTemplate` because their source payloads are different.

### Multiple AgentConfigs

Kelos can have many named `AgentConfig` resources. A task can reference one:
...
Kelos also supports composing multiple configs with `agentConfigRefs`:
...
- `agentsMD` values are concatenated.
- `plugins` are appended.
- `skills` are appended.
- `mcpServers` are merged by name; later entries win on name collisions.

Practical Cody split:

| Config | Purpose |
| --- | --- |
| `cody-base` | Shared Cody tone, Slack response rules, basic Alpheya context. |
| `cody-debugger` | Ops investigation workflow, cluster/debug tooling rules, PR expectations. |
| `cody-facts` | Optional tiny isolated config for toy or experimental behavior. |

For production Cody flows, composition is useful. For a toy experiment, keep it independent so it does not inherit the full debugger behavior by accident.

### Example: Experimental Slack Facts Task

Goal: keep the current Slack `@cody` behavior, but add a second Slack task that handles a command-style request:

```text
@cody facts tell me something
```

Minimal GitOps change:

1. Add a new `TaskSpawner` that matches `^facts\b`.
2. Add the same pattern to the existing catch-all debugger spawner's `excludePatterns`.
3. Do not change Kelos code or rebuild the image.

The exclusion matters because today's `cody-debug-slack` spawner matches every `@cody ...` mention with `.+`. Without an exclusion, both spawners would answer the same Slack message.
...
spec:
  ...
  taskTemplate:
    ...
    promptTemplate: |
      ...
      Reply with one short, mildly sassy, harmless tech/ops fact.
      Do not investigate the cluster.
      Do not open a PR.
      ...
      Reply once in the same thread with only the fact.
    ttlSecondsAfterFinished: 3600
```

Existing catch-all spawner change:
...
This example intentionally does not reference `cody-debugger` as an `AgentConfig`. A facts responder does not need the full ops-debugging operating manual, cluster workflow, repo lookup rules, or PR behavior. If the task should feel like Cody but stay lightweight, create a small separate `AgentConfig`; if it is only a prompt experiment, leave `AgentConfig` out.
...
For live schema validation without changing the cluster:

```bash
kubectl apply --dry-run=server -k non-prod/kelos
```

For fast real testing in non-prod, apply only the changed TaskSpawner manifests directly, then delete the experimental `TaskSpawner` after testing. This bypasses Flux and should be used only for prompt iteration, not as the final deployment path.
