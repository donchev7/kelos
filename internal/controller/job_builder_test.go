...
func TestBuildSessionRunner_DefaultsAllowPrivilegeEscalationFalse(t *testing.T) {
	builder := NewJobBuilder()
	session := &kelosv1alpha1.AgentSession{
		...
		Spec: kelosv1alpha1.AgentSessionSpec{
			TaskTemplateSnapshot: kelosv1alpha1.TaskTemplate{
				Type: AgentTypeCodex,
				Credentials: kelosv1alpha1.Credentials{
					Type: kelosv1alpha1.CredentialTypeNone,
				},
	...
	job, err := builder.BuildSessionRunner(session, nil, nil)
	if err != nil {
		t.Fatalf("BuildSessionRunner() error = %v", err)
	}
	csc := job.Spec.Template.Spec.Containers[0].SecurityContext
	if csc == nil || csc.AllowPrivilegeEscalation == nil {
		t.Fatalf("AllowPrivilegeEscalation was not defaulted: %+v", csc)
	}
...
func TestBuildSessionRunner_DoesNotOverrideAllowPrivilegeEscalation(t *testing.T) {
	builder := NewJobBuilder()
	session := &kelosv1alpha1.AgentSession{
		ObjectMeta: metav1.ObjectMeta{Name: "session", Namespace: "kelos-system"},
		Spec: kelosv1alpha1.AgentSessionSpec{
	...
	csc := job.Spec.Template.Spec.Containers[0].SecurityContext
	if csc == nil || csc.AllowPrivilegeEscalation == nil {
		t.Fatalf("AllowPrivilegeEscalation missing: %+v", csc)
	}
	if !*csc.AllowPrivilegeEscalation {
		t.Fatal("AllowPrivilegeEscalation was overwritten, want true")
	}
	if csc.ReadOnlyRootFilesystem == nil || !*csc.ReadOnlyRootFilesystem {
		t.Fatalf("ReadOnlyRootFilesystem was not preserved: %+v", csc)
	}
}
